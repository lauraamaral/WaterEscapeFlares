...
path = '/media/laura/LNRA_PHD/first_paper/M_dwarfs_Water/DATA/'
...
for ik, val in water_escP_flare.items():
    wf.append(val)

        
for ik, val in water_escP_stellar.items():
    ws.append(val)
...
CSs.append(axes[2,0].contour(X, Y,Zs[3], levels1 , colors= ('black','black','black','k')))
CSs.append(axes[3,0].contour(X, Y,Zs[5], levels1, colors= 'black'))
CSs.append(axes[4,0].contour(X, Y,Zs[9], levels1, colors= 'black'))
...
CSf.append(axes[2,1].contour(X, Y,Zf[3], levels1, colors=  ('black','black','black','k')))
CSf.append(axes[3,1].contour(X, Y,Zf[5], levels1, colors=  ('black','black','black','k')))
CSf.append(axes[4,1].contour(X, Y,Zf[9], levels1, colors=  ('black','black','k')))
...
plt.savefig('WaterEscapeAbsolute.pdf', bbox_inches="tight", dpi=300)
